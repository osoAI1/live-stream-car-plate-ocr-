...
class WebcamApp:
    def __init__(self, window, cap):
        ...
        self.correct_vehicles = 0
        self.total_license_plates = 0
        self.correct_license_plates = 0
    ...
    def update(self):
        ...
        if ret:
            ...
            self.total_vehicles += len(vehicles_in_frame)
            ...
            self.total_license_plates += len(license_plates)
            ...
            precision = self.correct_license_plates / self.total_license_plates if self.total_license_plates > 0 else 0
            # recall = self.correct_license_plates / self.total_vehicles if self.total_vehicles > 0 else 0

            cv2.putText(img, f'Precision: {precision:.2f}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            
            cv2.putText(img, f'Time: {elapsed_time:.2f}s', (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
